...
data_participation_details3['official_time'].apply(lambda x: datetime.datetime.utcfromtimestamp(x).strftime("%H:%M:%S")
                                                   if x > 0 else datetime.datetime.utcfromtimestamp(0).strftime("%H:%M:%S"))
data_participation_details3['net_time'].apply(lambda x: datetime.datetime.utcfromtimestamp(x).strftime("%H:%M:%S")
                                              if x > 0 else datetime.datetime.utcfromtimestamp(0).strftime("%H:%M:%S"))
data_participation_details3['team_idf'].apply(lambda x: None if pd.isna(x) else x)
...
with open('tmp-data_participation_details3.csv', 'r', encoding="utf-8") as f:
    ...
    cur.copy_from(f, 'participation_details', sep="%", columns=('partdet_id ', 'bib', 'official_time', 'net_time',
                                                                ...
                                                                'runner_id', 'ageclass_id', 'event_id'))
    con.commit()
